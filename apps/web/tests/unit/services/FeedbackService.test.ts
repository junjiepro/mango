...
describe('FeedbackService', () => {
  ...
  beforeEach(async () => {
    ...
    // 重新导入服务以获取新的实例
    const serviceModule = await import('@/services/FeedbackService');
    feedbackService = serviceModule.feedbackService;
  });
  ...
  describe('createFeedback', () => {
    it('应该成功创建正面反馈', async () => {
      ...
      mockSupabase
        ...
        .insert()
        .select()
        .single.mockResolvedValue(mockSuccessResponse(mockFeedback));
    ...
    it('应该在数据库错误时正确传播错误', async () => {
      ...
      mockSupabase
        ...
        .insert()
        .select()
        .single.mockResolvedValue(mockDatabaseError('Insert failed'));
  ...
  describe('getFeedbackByConversation', () => {
    it('应该返回正确数量的反馈并按时间排序', async () => {
      ...
      mockSupabase
        ...
        .select()
        .eq()
        .order.mockResolvedValue(mockSuccessResponse(mockFeedbackList));
    ...
    it('应该在没有反馈时返回空数组', async () => {
      mockSupabase.from().select().eq().order.mockResolvedValue(mockSuccessResponse([]));
  ...
  describe('getUserFeedback', () => {
    it('应该获取用户的反馈列表', async () => {
      ...
      mockSupabase
        ...
        .eq()
        .order()
        .limit.mockResolvedValue(mockSuccessResponse(mockFeedbackList));
    ...
    it('应该支持自定义 limit 参数', async () => {
      mockSupabase.auth.getUser.mockResolvedValue(mockAuthenticatedUser());
      mockSupabase.from().select().eq().order().limit.mockResolvedValue(mockSuccessResponse([]));
  ...
  describe('deleteFeedback', () => {
    ...
    it('应该在删除失败时抛出错误', async () => {
      ...
      await expect(feedbackService.deleteFeedback('feedback-123')).rejects.toMatchObject({
