 ...
 * @returns 是否需要刷新
 */
export function isUrlExpired(url: string | undefined | null, bufferSeconds: number = 3600): boolean {
  // 如果 URL 为空或无效，认为需要刷新
  ...
  try {
    ...
    // URL 解析失败，认为需要刷新
    logger.debug('Failed to parse URL for expiry check, will refresh', { url: url?.substring(0, 50) })
    return true
  }
}
