...
export function useStreamingMessage(conversationId: string | null) {
  ...
  const updateToolCall = useCallback((messageId: string, toolCall: ToolCall) => {
    setStreamingMessages((prev) => {
      ...
      if (existing) {
        ...
        const updatedToolCalls = existingIndex >= 0
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'tool_call_result' }, (payload) => {
      ...
      // 如果是图片生成成功，自动添加到文件列表
      if (data.status === 'success' && data.tool === 'image_generate' && data.result?.url) {
  ...
  }, [conversationId, supabase, updateStreamingMessage, addFileToStreamingMessage, updateToolCall, clearStreamingMessage])
