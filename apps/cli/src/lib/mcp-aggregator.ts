...
import { ServerClient } from '../types/mcp-config';
...
export class MCPAggregator {
  ...
  private transport: StreamableHTTPTransport | undefined;
   ...
   * Get the aggregator server instance
   */
  public async getConnectedTransport(): Promise<StreamableHTTPTransport> {
    const mcpServer = this.server;

    if (!mcpServer.isConnected() || !this.transport) {
  ...
  private setupRequestHandlers(): void {
    ...
    this.server.server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => {
  ...
  private async aggregateTools(): Promise<any[]> {
    ...
    const toolPromises = Array.from(this.clients.values()).map(async (serverClient) => {
      try {
        ...
        console.error(`Failed to list tools from server ${serverClient.name}:`, error);
      }
      return null;
    });
  ...
  private async callTool(params: any): Promise<any> {
    ...
    if (!serverClient) {
      throw new McpError(ErrorCode.InvalidRequest, `Server not available for tool: ${name}`);
    }
  ...
  private async aggregateResources(): Promise<any[]> {
    ...
    const resourcePromises = Array.from(this.clients.values()).map(async (serverClient) => {
      try {
        ...
        console.error(`Failed to list resources from server ${serverClient.name}:`, error);
      }
      return null;
    });
  ...
  private async aggregateResourceTemplates(): Promise<any[]> {
    ...
    const templatePromises = Array.from(this.clients.values()).map(async (serverClient) => {
      try {
        ...
        console.error(`Failed to list resource templates from server ${serverClient.name}:`, error);
      }
      return null;
    });
      ...
      if (result) {
        for (const template of result.templates) {
          ...
          if (protocolMatch) {
            ...
            this.resourceProtocolMap.set(`template:${standardizedTemplate}`, originalProtocol);
          } else {
  ...
  private async readResource(uri: string): Promise<any> {
    ...
    if (!targetClient) {
      throw new McpError(ErrorCode.InvalidRequest, `Server not found: ${serverName}`);
    }
  ...
  private async aggregatePrompts(): Promise<any[]> {
    ...
    const promptPromises = Array.from(this.clients.values()).map(async (serverClient) => {
      try {
        ...
        console.error(`Failed to list prompts from server ${serverClient.name}:`, error);
      }
      return [];
    });
    ...
    for (const serverClient of this.clients.values()) {
      try {
        ...
        if (promptsResponse && Array.isArray(promptsResponse.prompts)) {
          const hasPrompt = promptsResponse.prompts.some((p: any) => p.name === name);
          if (hasPrompt) {
