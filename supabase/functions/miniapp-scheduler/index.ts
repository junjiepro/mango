  ...
  try {
    ...
    for (const installation of installations || []) {
      ...
      for (const trigger of triggers) {
        ...
        const shouldTrigger = await checkScheduleTrigger(installation.id, trigger, now, supabase);
...
async function checkScheduleTrigger(
  ...
  const lastRunTime = lastTrigger?.value ? new Date(lastTrigger.value) : new Date(0);
